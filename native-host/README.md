...
| File | Role |
|---|---|
| `bridge.mts` | The stdio host (reads/writes length-prefixed JSON) — bundle entry. |
| `protocol.mts` | The message contract, imported by the extension too — see below. |
| `paths.mts` | Resolves the shared config dir (must match the Electron app). |
| `media-download.mts` | Shared best-effort still-image downloader (SSRF guard, size/time caps) — also a bundle entry (see below). |
| `config-recovery.mts` | Save-folder recovery + destructive-op gating (pure functions). |
| `install.mts` | Registers/removes the host manifest for Chrome/Edge/Chromium/Brave/Vivaldi (#210). |
| `com.hologram.host.json` | Reference template (the real one is generated by `install.mts`). |

The sources are ESM and carry types Node strips at runtime — they run un-built,
with no compile step. `install.mts` is loaded as-is by the Electron app, whose
own bundle is CommonJS: Node's `require(esm)` makes that synchronous load legal,
which is what let this directory stop being CommonJS in #1052. See
`native-host/tsconfig.json` for the full rationale, including what the CJS shape
was costing (tsc reads no exports off `module.exports`, so 16 test suites were
excluded from type-checking outright).
...
`bridge.mts` and the modules it requires are **bundled** into
...
`install.mts` deploys into the config dir (`%APPDATA%\Hologram` on Windows, see
`paths.mts`) for the generated launcher to run. The
...
The same build script also bundles `media-download.mts` alone into
...
node native-host/install.mts <extensionId>    # register, allowing this extension
node native-host/install.mts uninstall        # remove
```
