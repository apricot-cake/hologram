...
## 作者まわり（クラスタ）
- **著者プロフィールの表示**（抽出は実装済み＝サイドカーに `avatar`／`followers`／`authorCreatedAt` 保存済み。全PFアバター、フォロワー/作成日は Bluesky/Misskey/Mastodon のみ・X/pixiv は公開APIに無く graceful 隠し）。残るはインスペクタ/投稿者ビューでの表示。
