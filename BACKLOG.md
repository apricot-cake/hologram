...
- **着地済み**: Vite 移行（esbuild 廃止・段階0-2）／島17個（settings/sidebar-tags/query-chips/tabs/collections/searchbox=react-aria ComboBox/posters/post-card=テンプレート島/grid=仮想化list/toolbar/context-menu/kind-menu/filter-popover/qf-pop/lightbox/inspector/edit-overlay）＋共有ストア window.corpusStore。フィルタ系（値フライアウト qfPop＋日付/エンゲージ/ポスター日付範囲ポップオーバー）は完了。**詳細/インスペクタパネル（post/poster）と一括タグ追加モーダルも完了＝インライン タグ編集/タグピッカーを共通 `_shared/TagEditor.jsx` へ一本化**（React化=目的1の実例）。Reactランタイムは全島から外部化し共有 vendor-react.js に一本化済み（c3269d4）。**検索ボックス＋サジェストも完了（スライスm・f0b206c）＝react-aria-components 導入・searchbox 島が input DOM を所有・値源は corpusStore searchQuery・旧 suggest 島は吸収削除**（実装知はメモリ `corpus-react-settings-pilot`）。#filterRows のバッジ（`renderFilterBadges`）は純粋な派生テキスト＋クラス切替でドリフトリスクが無く、React化の対象外と判断（現状維持）。
- **残タスク**:
  - グリッド完全React化＝#postGrid を viewer.js→React 所有＋仮想化（深スクロールの線形劣化解消。回帰リスク最大＝スライス群で慎重に）。**スライス1完了（2026-07-02）＝list ビューを masonic 仮想化で React 所有**: grid 島＋`window.corpusGrid` ブリッジ・9042件→DOM 22セル・スクロール復元は renderLimit 復元を廃し scrollTop 直復元（リロード跨ぎ実機PASS）・実機フルマトリクス PASS（選択/シフト範囲/リサイクル跨ぎ永続/テキスト展開リフロー/インスペクタリング/ライトボックス/右クリック/ソート/スライダー/ビュー切替ハンドオフ/空状態/ブラウズモード切替）。実装知（ホスト分離・itemsKey vs paint・深ジャンプ一括計測バースト等）はメモリ `corpus-react-settings-pilot`。**残り＝tile→card の順で同基盤へ**: tile=均等正方（columnWidth=tileSize が auto-fill minmax と同挙動＝PoC 実測）／card=本丸（aspect 予約 shotW/H+imgAspect を高さ見積りに接続・cardSize スライダー対応）。card 反転で renderLimit/reconcile/手書きマサンリー/sentinel/fast path を丸ごと削除（タブ保存の `_renderLimit` は legacy ビュー用に残置→全ビュー反転後に除去）。
