...
- **投稿者ビュー（実装済み 2026-06-18）**: コンテンツ上部の「投稿/投稿者」モード切替トグル（`browseMode`）で投稿者グリッドへ。カード=アバター/モノグラム＋投稿者名＋@＋PF＋投稿数。クリック=インスペクタ（投稿者プロフィール）／ダブルクリック=投稿モードでその投稿者の `user` フィルタ。投稿者モードのサイドバーは並び順（投稿数/名前/最新投稿）＋プラットフォーム絞り＋お気に入り絞り＋検索。お気に入り＝カード/インスペクタの★トグル（`poster-favorites.json` に永続化）。詳細は architecture.md「ビューア機能」。**残**: 投稿者の**名前付きフォルダ**（複数・お気に入りの上位概念）／投稿者への**タグ付け**／作品タググループを第一級概念へ。
