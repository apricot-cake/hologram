...
## UI / 見た目
- **タグ付け起動ボタンの配置とデザインを再考**: 現状はサイドバー最上部の横長ボタン（`.sb-feature`）。`--chip-bg` 塗りだと「浮いた箱感」、透明行＋mutedアイコンだと「ボタン感がない」（馴染みすぎ）。両者の中間＝馴染むがボタンと分かる見た目＋そもそもの配置から考え直す（2026-06-18 ユーザー指摘）。
