...
- **Tailwind CSS + shadcn/ui への移行**: 手書き CSS（index.html 内・ガラス調）と自前/headless 部品を Tailwind ユーティリティ＋shadcn/ui（copy-in 型・Radix ベース）へ載せ替える。**要整合**: ①「UI はガラス維持・styled kit 不採用」（2026-06-30 スパイク確定・React化節）＝shadcn は copy-in で全面テーマ可＝**ガラストークンで見た目を保ったまま実装層だけ移行**なら両立しうる（見た目も変えるなら別の決定＝要確認）②react-aria-components 採用済み（searchbox）と Radix 系の共存 or 置換 ③DESIGN.md 全面改稿が伴う（UI変更は DESIGN.md 準拠ルール）④Claude Design キットとの噛み合わせ（キット側スタックに寄るなら design→code 翻訳コストが下がる＝移行動機そのもの）。時期＝最終形B（単一Reactアプリ化・Vite 単一バンドル）後が自然＝島 IIFE のままでは Tailwind のビルド/パージ統合が半端。
- **サイドカー JSON をやめ SQLite へ本格移行（真実源の DB 化）**: 「採用#4 SQLite 派生インデックス」（真実源は JSON のまま）を超えて、投稿メタデータの**真実源自体**を SQLite へ。画像/メディアはファイルのまま＝DB はメタデータのみが現実線。**要整合**: ①「ファイルベース真実源」は恒久設計制約として複数決定の前提（採用#4・export/import・バックアップミラー・write-once サイドカー）＝制約自体の撤回になる ②native host（Chrome spawn の別プロセス）が保存先へ直接書く経路＝多プロセス同時書きの再設計（WAL 前提でも要検討・ホストは JSON を書き app が取込む2段構えも選択肢）③2026-06-23 消失事故対策（冗長ポインタ・degraded ガード・「壊れたら再構築」）の防御を DB 前提で再構築（バックアップ形式・破損復旧）。**段階**: まず採用#4（派生インデックス）で SQLite 層を実戦投入→運用知見の上で真実源の移行を最終判断、が安全経路。
