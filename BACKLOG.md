...
- **【確定#1・任意フォローアップ】組織 JSON 読み側の degraded 化**＝「ファイルは存在するが parse 失敗」を空欠如と区別し degraded 扱いで空 persist を抑止（clear-all の degraded ガードと同思想・外部破損や既存 torn ファイルへの多重防御）。書き込み側のアトミック化（`writeJsonAtomicSync`/`writeOrgAtomic`）は対応済み＝これはその上の多重防御。
