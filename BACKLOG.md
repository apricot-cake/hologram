...
- **投稿者ビュー（実装済み 2026-06-18）**: コンテンツ上部の「投稿/投稿者」モード切替トグル（`browseMode`）で投稿者グリッドへ。カード=アバター/モノグラム＋投稿者名＋@＋PF＋投稿数。クリック=インスペクタ（投稿者プロフィール）／ダブルクリック=投稿モードでその投稿者の `user` フィルタ。投稿者モードのサイドバーは並び順（投稿数/名前/最新投稿）＋プラットフォーム絞り＋検索。詳細は architecture.md「ビューア機能」。**残**: 投稿者へのタグ付け／投稿者のフォルダ（お気に入り兼用）／作品タググループを第一級概念へ。
