...
- **残タスク**:
  - グリッド完全React化＝#postGrid を viewer.js→React 所有＋仮想化（深スクロールの線形劣化解消。回帰リスク最大＝スライス群で慎重に）。**masonic 4.1.0 は導入・実行時PoC済み（2026-07-02）**: file://+厳格CSP+vendor-react外部化+独自スクロールコンテナ（useMasonry+自前scroller）で動作確認・2000件でDOM16セル・バンドル10KB/require0（詳細な罠と設計要点はメモリ `corpus-react-settings-pilot`）。**攻め方の骨子**: ①同一DOM契約（.post-card/data-index/data-key）で #postGrid 上の既存委譲を無改修存続 ②スクロール復元は renderLimit 復元をやめ scrollTop 直復元＋positioner リセット時の再同期（PoC で判明した罠） ③renderLimit/reconcile/手書きマサンリー/sentinel は仮想化で丸ごと消える（タブ保存の _renderLimit は移行/無視） ④aspect 予約（shotW/H+imgAspect キャッシュ）は itemHeightEstimate/高さ計算の種として温存。着手時に最初の反転ビュー（list=構造単純 vs card=本丸）を選んで段階分割。
