...
- **投稿者ビュー（最小・実装済み 2026-06-18）**: コンテンツ上部の「投稿/投稿者」モード切替トグル（`browseMode`）で投稿者グリッドへ。カード=アバター/モノグラム＋投稿者名＋@＋PF＋投稿数（投稿数順）。クリック=インスペクタ（投稿者プロフィール）／ダブルクリック=投稿モードでその投稿者の `user` フィルタ。詳細は architecture.md「ビューア機能」。**残**: 投稿者へのタグ付け／投稿者のフォルダ（お気に入り兼用）／作品タググループを第一級概念へ。投稿者カードのソート切替（名前順等）・件数の多い投稿者だけ等の絞り。
- **同一投稿者がSNS別にばらける→名寄せ**: 同じ人でもプラットフォームが違うと別グループになる。手動で紐づける機能＋自動で同一人物を提案する機能（名寄せ候補）。呼称は「投稿者」に確定済み（DESIGN.md「用語」）。
- **投稿者モードのサイドバー**: 暫定＝投稿専用UI（フィルタ/ソート/密度/クエリビルダー/タグ付け/ワークスペース）を隠し検索だけ残す。**残**: 投稿者向けのフィルタ（プラットフォーム絞り・ソート切替＝投稿数/名前/最新）をサイドバーに足すか要検討。
