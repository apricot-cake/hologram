...
- **複数ウィンドウの許可**: 単一ウィンドウ制約を緩める（現状＝`main.js:1270` requestSingleInstanceLock で単一強制・2起動目は既存ウィンドウを focus のみ）。下「操作性」の画像ミドルクリック＝新規タブ案とは別軸（タブは既存・ウィンドウ複数化は未）。
...
- **画像のミドルクリックを「新規タブで開く」に統一（ブラウザライク・要設計 2026-07-03 判明）**: 現状はカード画像のミドルクリックで単一画像ウィンドウ（`openImageWindow`）。既存タブ＝フィルタ状態タブ（tree/search/sort）で画像コンテンツを表示する器が無い＝「画像タブ」新種別（永続化スキーマ変更）が要る。上「画面フィット表示＋インスペクタ（Eagle 風）」の詳細ビュー設計と同じ画面＝合わせて設計してから着手。
