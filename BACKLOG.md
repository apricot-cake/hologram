...
## 作者まわり（クラスタ）
- **著者プロフィールの表示**: インスペクタに実装済み＝フォロワー数・登録日のテキスト行＋投稿者名の横に円形アバター。アバターはオフライン保存方式（キャプチャ時に native-host がローカルへDL→ `avatarFile` を psimg:// で表示。pixiv アバターは Referer ゲート対応済み。X/pixiv のフォロワー/作成日は公開APIに無く graceful 隠し）。**残**: ①取込(import-posts)・backfill(backfill-metadata.js)・既存データの経路はアバター画像を未DL＝`avatarFile` 無しでアバターだけ出ない（フォロワー/登録日のテキストは出る）→ 両経路にアバターDLを足す。②投稿者ビュー（下記クラスタ）が出来たらそちらにもアバターを出す。
