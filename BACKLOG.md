...
- **React 島移行の後始末＝旧 #panelSettings 撤去（保留中・React 版本採用済み）**: 設定モーダルは React 島へ全移行済み（main・全6節）だが、`app/renderer/index.html` の旧 `#panelSettings` マークアップと viewer.js の旧設定ハンドラ群は**非表示で温存**中（動作・見た目に影響なし）。安全に一括撤去するには ①`importZipInput`（空状態の取込ボタン `emptyImportBtn` と共有・817行）の再配置 ②ノーガードな旧ハンドラ（langSelect/tileOverlayToggle/exportZip/importZip/importImages/clearData/resetDeleteConfirm 等＝要素消失で throw）の除去 ③`setupSaveFolder`/`setupBackup`（レール `#mirrorStatus` は残す）/`setupTrash` の整理、が要る。React→vanilla ブリッジ（`window.corpusViewer.*`）と共有確認オーバーレイ（`#confirmOverlay`）は残す。
