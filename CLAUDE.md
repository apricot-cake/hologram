...
- **本体作業ツリーの同期と拡張機能の反映は確認なしで行う**: 共有`main`の更新を本体へ取り込む前に未コミット変更と差分を確認し、安全に統合できると分かれば同期する。**拡張は日常のChromeが本体の`extension/.output/chrome-mv3`を読む1本構成**（`outDirTemplate`）。**反映は`npm run build:ext`一本で、リロードのクリックは要らない**（2026-07-31 #650＝ビルドが`~/.hologram/extension-build.json`に識別子を書き、ネイティブホストが全返信に乗せ、拡張が自分で`chrome.runtime.reload()`して開いていたタブも再読み込みする。保存中・キャプチャUI表示中・一括取込中は見送られる）。**ホットリロード用の開発サーバー（`dev:ext`）は撤去済み**（2026-07-31 #675＝`build:ext`が0クリックへ追いついたため、開発サーバーを止めたまま戻し忘れると普段使いの拡張が黙って死ぬ危険ごと道具を無くした）。手順はskill `verify-extension`、理由はdocs/build.md「拡張機能の開発・配布」。並行作業との衝突確認は維持するが、ユーザーへの可否確認は不要（2026-07-25）。
