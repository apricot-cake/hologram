...
- **Liquid Glass をライトのガラス面全般へ展開**: 表示切替つまみは試作済み（淡い色のぼかし＋屈折フリンジ＋艶で白地でもガラス感）。同方式をフライアウト/ポップ/パネル等の他のライトのガラス面にも展開するか（つまみの仕上がり確定後）。※色を使う＝モノトーン基調の意図的な material 例外、確定したら DESIGN.md に記録。
