...
- **フライアウトのホバー表示**: サイドバーのフィルタ行にカーソルを乗せると（約140ms後）フライアウトが開く実装にした。スクロールや誤爆で"じゃま"にならないか、開閉のキビキビ感を実機で。じゃまなら撤回（クリックは従来どおり残してある）。
...
### タグ付けの仕上げ（2軸タグ付けは実装済み＝DESIGN.md「操作語彙」参照。ユーザーがレビュー中＝優先クラスタ）
- **スタンプのパレットにグループ整理＋検索 [最優先]**: 現状は全タグのフラットな横スクロールで「付与しづらい」。タググループでの整理（節分け/グループ絞り込み）＋タグ検索を入れる。グループ横断タグ検索もここで。
- **カード編集モーダルが貧弱**: クリックで出るのが小さな「タグを入力/追加/保存」モーダルだけ。既存タグの一覧表示・グループからの候補・タグ候補補完など、ちゃんとした編集UIに。
- **スタンプ付与時のアクセント枠が変**: `.post-card.stamp-on` のリング（inset 2px accent-soft）＋✓バッジの見た目を見直し。
- **祝祭エフェクト不要かも**: スタンプ付与時の celebrate を外す/控えめにする（要・最終判断）。
...
- **呼称の統一を決める**: 「作者/ユーザー/投稿者」等の用語ゆれを正式に1つへ確定（全UI文言・i18n を揃える）。上の表現見直しと一体で。
- **同一投稿者がSNS別にばらける→名寄せ**: 同じ人でもプラットフォームが違うと別グループになる。手動で紐づける機能＋自動で同一人物を提案する機能（名寄せ候補）。
