...
- **本文の全文検索 [競合調査2026-06-17・最優先]**: 投稿本文を横断検索（本文/タイトル/pixivキャプション）。本文テキストは既に全PF分サイドカーに保存済み（`metadata.js` の `rec.text`）＝OCR/AI不要・既存データを引くだけでローカル完結＝オフライン方針と無衝突。競合（Karakeep/Eagle）が全文検索にOCR/AIを要するのは生画像しか持たないため＝Corpus は構造化メタを持つので別格に低コスト高価値。※コマンドパレット（操作/タブ移動ランチャ）とは別物＝こちらは本文の中身検索。
- **スマートフォルダ（保存できる動的検索）[競合調査2026-06-17・次点]**: 今のクエリビルダーの条件を「名前を付けて保存」できる動的フォルダに（前例=Eagle）。既存クエリビルダーの自然な延長で軽い。新規タブ運用・上の「接合の自由化」との整合は要設計。
