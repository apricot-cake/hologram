...
### サイドバー / フライアウト（2026-06-15 依頼）
- **フィルタの「取得元」セクションとラベルを廃止**: サイドバーのフィルタにある取得元（インスタンス/サーバー）の見出し・ラベルを撤去。※実装前に対象の正確な箇所（取得元の表示・ラベル）を要特定。
- **プラットフォームのフライアウトに「プラットフォームなし」項目を追加**: platform 未設定の投稿を絞り込めるよう、プラットフォームのフライアウトに該当項目を足す。
- **ホバーでフライアウトを開く挙動を廃止**: クリックのみに戻す（カーソルを乗せて約140msで開く実装を撤回）。
- **[bug] フライアウトの絞り込み入力でフライアウトが消える**: 絞り込みのテキストを入力するとフライアウトが閉じてしまうことがある（focus/blur まわりの取り回し）。

### レイアウト（2026-06-15 提案・Claude アプリ参照）
- **クエリビルダーを全幅トップバー化＋サイドバーを浮かせる**: ①クエリビルダーを左端まで伸ばし、横幅いっぱいの上部バーにして**サイドバーの上を貫く**（最上段をクエリビルダーが全幅で占有）。サイドバーはその下から始める（＝サイドバーはクエリビルダーの下でよい）。②そのうえで Claude デスクトップアプリのサイドバーのように、**サイドバー自体を角丸・余白付きで「ウィンドウっぽく」浮かせて独立**させる案。※"浮かせる/独立"は要否を検討（ユーザーは「？」付きで提案）。前例＝Claude アプリ（UI判断は実プロダクトに錨＝[[design-real-product-anchoring]]・DESIGN.md に記録のこと）。

### UI / 見た目
- **ライトのガラス「ふち」感をダークに揃える**: ライトではガラスのふち（リム/エッジ）が出ない。ダークとなるべく見た目を揃える（表示切替つまみは対応済み＝残るはフライアウト/パネル/ポップ等のガラス面）。
- シェブロン過密・チープ: タグ以外/タググループのシェブロン要否・ホバー時のみ表示等。
- 設定検索は現状セクション単位の絞り込み（該当項目を含む節ごと表示）。項目単位のハイライトまで要るか要検討。
- **アクティブフィルタ数バッジの刷新**: サイドバーのフィルタ行に出る件数バッジ（数字）が「ダサい」。形/色/出し方を見直す（モノトーン基調・DESIGN.md準拠）。
- **あいまい検索のバー内統合**: ①フライアウトの絞り込み入力バーに「あいまい/通常」プルダウンをバー内に。②メインの検索バーも、別 select ではなくバー内にあいまい切替を統合。
- **ミラーリングステータスの移設**: ミラー状況の表示を、サイドバー下部の設定アイコンのエリアへ寄せる。
