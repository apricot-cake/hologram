...
> **【ナレーション】ツール実行前後の経過説明は、平易な日本語で。** 中核＝**固有名詞（ファイル名・コマンド名・操作名・API 名）を省略しない**（「設定ファイル」ではなく `config.json`、「レジストリ操作」ではなく `reg add`）。「平易」は語選びを分かりやすくの意で、説明を増やすことではない。分かりにくい用語は短い言い換えを任意で添えてよい。対象はインラインのナレーションのみ。
...
> **【モデル】モデルの分業・切替提案はしない**。いまアクティブなモデルのまま設計も実装も
> 進める。どのモデルを使うべきかの提案・催促をユーザーに投げない（把握しきれず無益なため
> 廃止＝2026-06-13）。

> **【バックログ運用】未完了・先送りの依頼は必ず
> `~/.claude/projects/C--Users-apricot---------corpus/memory/corpus-ui-backlog.md` へ追記する**
...
- `app/` — Electron デスクトップアプリ。`main.js`/`preload.js`/`renderer/`（`index.html`・`viewer.js`・`i18n.js`）、`lib-archive.js`（ZIP入出力）、`lib-index.js`（保存先サイドカーの index＝filename+mtimeMs で記録をキャッシュ。`listPosts` を非同期・O(changed) 化し `.index.json` スナップショットで起動も高速化。更新は差分IPC（list-posts-delta）＋fs.watch の変更ファイル名ヒントで対象サイドカーだけ再走査（applyChanges）＝実測 ~1ms。Electron非依存＝node でテスト可）、`vendor/jszip.min.js`。サイドカー走査で閲覧、保存先選択・拡張ID設定・ホスト自動登録。画像は `psimg://` プロトコルで遅延読込
