...
### タグ付けの残（2軸タグ付けは実装済み＝DESIGN.md「操作語彙」参照）
- **グループ横断タグ検索**: 複数グループにまたがるタグの検索/絞り込み（2軸タグ付けでは別途とした分）。
- スタンプのパレットに**グループ単位の絞り込み/並び**を入れるか要検討（現状は全タグのフラットな横スクロール）。
...
- **クエリビルダーのチップにグリフ**: 上部の絞り込みピルに、サイドバーのフィルタセクションと同じ種別グリフ（アイコン）を付けて対応を取る。
- **フライアウトをホバー表示に（試す）**: サイドバー行のフライアウトをクリックではなくホバーで開く案。ワンクリック減るのが狙い。じゃまになったら撤回（要・実機の体感確認）。
- **設定画面の目次**: 設定画面の横に目次（セクションへのジャンプ）を付ける。
...
### その他
- **設定項目の検索機能**: 設定画面内の項目を検索で絞り込めるように。
