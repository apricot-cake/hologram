...
- **本体作業ツリーの同期と拡張機能の反映は確認なしで行う**: 共有`main`の更新を本体へ取り込む前に未コミット変更と差分を確認し、安全に統合できると分かれば同期する。**拡張の開発・検証は`npm run dev:ext`が起動する開発用ブラウザで行う**＝ホットリロードに任せ、手動の再読み込みをしない（永続プロファイルでログイン維持・CDPは:9223）。日常のChromeが読むのは`extension/.output/chrome-mv3`（production）で、そちらへ届ける時だけ`build:ext`＋リロード1回をユーザーに依頼する。**dev出力を日常のChromeへ読み込ませない**（devサーバーが止まると普段使いの拡張ごと沈黙する・2026-07-26 #362）。手順はskill `verify-extension`、理由はdocs/build.md「拡張機能の開発・配布」。並行作業との衝突確認は維持するが、ユーザーへの可否確認は不要（2026-07-25）。
