...
> **【モデル分業】反復的な作業は1セッション1モデルで通す（per-stepで切り替えない）**。
> 設計と実装を小さく往復する UI 磨き・小機能・調整・バグ直しは、いま動いているモデルのまま
> 設計も実装も流してよい（コード編集を禁じない・切替を催促しない）。**正式な分業は大きな
> 非対称タスクだけ**＝広い推論が要るアーキ/IA 設計（Opus）→ 合意後に大量の機械作業（Sonnet）、
> のように性質がはっきり分かれる時のみ、その境目で一度だけ切替を提案する。（2026-06-13 改定）
