...
- **本物の屈折(#liquidGlass)を他のガラス面へ展開**: フライアウト/メニュー(`.fold-menu`)とライトつまみは適用・確定済み（DESIGN.md記録済み）。残＝`.qf-popover`（日付/エンゲージ）・設定パネル・インスペクタへ展開するか。大面積はGPU負荷を見つつ（フライアウトは軽いが全画面級は別）。要否は実機で判断。
