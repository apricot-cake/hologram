...
> **【試行】セッション切り替えの提案（2026-06-13 開始 / 2026-06-27 に評価）**：自然な区切りで、
> ①ユーザーが無関係な別プロジェクト/話題へ pivot した、②1セッションが毛色の違うタスクを跨いで
> 明らかに肥大した——のいずれかのときだけ「新セッションにするのも手」と**一度だけ軽く**添える。
> context 使用率は監視しない（harness の自動コンパクション任せ＝私には％が見えない）。nag 禁止。
> ユーザーがモバイル/リモートだと言った場合は提案しない（ローカル新規はデスクトップでしか作れず
> 動けないため）。**評価の想起は私の責任**：`currentDate` が 2026-06-27 以降になった最初の
> セッションで、この試行が有用だったか（残す/調整/廃止）を私から評価提起する。
