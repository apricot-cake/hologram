...
- 並列運用は1ラウンド完了。進行中ワーカー: **なし**。
- 直近: poster-tags①（投稿者タグ付け基盤）をマージ済み（`7ef9e03`）。実機確認はユーザーに委譲。要確認: タグ編集を常時表示にした判断・undo非対応。
- 次の一手: ユーザーに poster-tags の感触と次タスクを確認してから新ラウンド。**無理に並列化しない**（viewer.js を取り合わない独立2本が自然に揃ったときだけ発動）。
