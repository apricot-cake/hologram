...
- **ホバー保存の描画回帰**: `npm run test:overlay-visual`＝使い捨てChromeへ本番ビルドの拡張を読み込み、X形式の画面でボタンと画像のスクロール位置・固定ヘッダー/モーダルの重なりを実ブラウザで検査する（重なりの判定点は**ポインタ**＝画像の上端がヘッダーに潜ってもカーソルが画像上ならボタンは残り、カーソル自体がヘッダーへ乗ったら消える）。ホバー配置や重なりを変えたら必ず実行する。
