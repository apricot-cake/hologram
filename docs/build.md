...
- **窓の見分けはタイトルでなく1行目**＝開く時に `Hologram dev:ext` というタイトルを付けるが、**サーバーが走り出すと `C:\WINDOWS\system32\cmd.exe` に戻る**（cmd は実行中のコマンドラインでコンソールタイトルを上書きし、npm から wxt へ届くまでに cmd の層が挟まる＝2026-08-04 実測）。窓の1行目に出る `[hologram] development build folder: …` が識別子＝隣のプロダクトの dev サーバー窓と並んでもこれで区別できる。**タイトルで探して見つからないのは異常ではない**。
