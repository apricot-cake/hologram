...
**新しい依存を伴うマージでは、ビルドの前に依存を入れる**（#897）。そのマージで `extension/package-lock.json` が動いた時だけ `npm --prefix extension ci` を挟む（動いていなければこれまでどおり素通し＝所要は延びない）。`install` でなく `ci` なのは、無人で走る経路がコミット済みのロックファイルを書き換えないため。**ルートの `package-lock.json` は報告するだけで入れない**＝`npm ci` は先に `node_modules` を消しにいき、日常のアプリが実行中だとその削除が途中で失敗して依存の欠けたツリーが残る（本ファイル冒頭の警告と同じ事故）。ビルドがそれで落ちた時は、アプリを閉じて `npm run setup`。
...
- **窓を開くのはユーザーの画面を奪う**＝だから「起動済みを見てから」であり、本当に必要な時は開いてよい（開いた事実を一行添える）。ユーザーが前面で使っている Chrome からフォーカスを取らない、が守りたいことの本体。
...
これは `node` を直接呼ぶ（解除は `npm run ext:dev:register -- uninstall`）。⚠️**かつては一度きりのスケジュールタスク経由だったが、その理由（コンテナ内の `reg add` は実 Chrome から見えない）は 2026-08-06 に失効し**（#1003）、**タスク経路は 2026-08-07 に撤去した**（#1006）。**登録の後、スクリプト自身が `reg query` で5つのキーを読み戻して各行の可否を出す**（旧記述「ここから確かめることはできない」は失効＝HKCU への書きも読みも実体）。読み戻しが合わなければ非ゼロ終了する。登録先は `~/.hologram-dev/`（ランチャーが `HOLOGRAM_CONFIG_DIR` を固定するので、Chrome が起動した bridge は実ライブラリを見られない）。
...
⚠️**生死を PowerShell から見るときは宛先を `127.0.0.1` と書く**＝Chromium の `--remote-debugging-port` は **IPv4 の `127.0.0.1` にだけバインドする**。PowerShell / .NET の `localhost` は `::1`（IPv6）へ解決されるので、`Test-NetConnection localhost 9222` のような判定は**開いているポートを「閉じている」と報告する**（`scripts/cdp-verify.cts` は Node の解決順で通るため、この食い違いは PowerShell 側でだけ出る）。⚠️**上の「開発実行」の dev server とは向きが逆**＝あちらは `::1` にだけバインドするので `127.0.0.1` を決め打ちすると空振りする。**同じ流儀で書かない**＝どちらも「相手が実際にバインドしている綴りへ合わせる」が正で、片方の作法をもう片方へ持ち込むと、どちらの場合も「動いているのに落ちている」と報告する。
