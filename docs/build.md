...
2. **`~/.hologram/extension-build.json` にそのビルドの識別子を書く**。ネイティブメッセージングホストは**全ての返信にこの識別子を乗せる**ので、拡張は保存・保存済みバッジの問い合わせ・ログ中継のどれかが往復するたびに「ディスク上のビルドが自分と違う」ことを知り、`chrome.runtime.reload()` を呼ぶ。リロード後は**開いていたタブも自動で再読み込みされる**ので、#594 の孤児タブも残らない。
