...
## バックログ着手時のスキャン（並列するか solo か）

並列は「探して作る」ものでなく「揃っていたら使う」もの。BACKLOG から次の作業を取るとき一度だけ次を見る（**既定は solo**）:

- **実装**: 次に取る2〜3タスクで、`viewer.js`/`index.html` を取り合わない独立ペアが自然に揃っているか。揃えば worktree 隔離ワーカーで並列、無ければ solo（corpus の実装は UI 集中＝大半が solo）。
- **設計・調査・レビュー**: ここがファンアウトの効きどころ（ultracode/workflow）。ファイル衝突が無いので積極的に並列化してよい。
- **ガード**: 並列化のために仕事を割らない。独立が自然に揃ったときだけ起こす（無理に並列化しない）。
