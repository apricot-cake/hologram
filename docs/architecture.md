...
- `renderer/`（`index.html`・`.ts`群＝`viewer.ts`が状態/オーケストレーション/IPC呼び出しの中核、`store.ts`ほか単機能サービス（`tags.ts`/`selection.ts`/`query.ts`/`records.ts`等）に段階抽出済み・`design-tokens.css`）。描画自体は下記`islands/`のReactコンポーネントが100%所有し、viewer.tsとは共有ストア`window.corpusStore`経由で連携（push型のモデル注入は撤去済み＝島は自己購読・メモリ`corpus-react-settings-pilot`）
