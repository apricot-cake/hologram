...
## 射程の原則（取込の3段構造・#190 で確定）

Corpus の名乗り・本業は「SNS 投稿のライブラリ」。ただし入れ物としては寛容で、取込は対象の性質に応じた3段のいずれかに収まる。個別機能の採否はこの3段のどれに属するかで判定する（2026-07-18 確定）。

1. **SNS 投稿＝丸ごと保存**（本業）: フロー型（流れて消える）コンテンツ。投稿者・リアクション等の構造化メタデータ付きの一級市民。
2. **ローカルファイル＝実体保存**: ローカル取込・要素キャプチャ等。メディア（画像・動画）はビューア・サムネイル込みのフル体験、それ以外の任意ファイル（PDF・配布資料等）は「収蔵品」＝保存・タグ・フォルダ・ファイル名検索・OS 既定アプリで開く＋安価なプレビューまで、の2階級で受ける（2026-07-19 に「画像・メディアファイル」から拡張＝#190 コメント・実装 #236）。
3. **その他あらゆる URL＝カード保存**: ストック型（プラットフォームに据え置かれ再訪される）コンテンツ。OGP（タイトル・説明文・サムネ）＋ユーザー注釈（タグ・メモ）まで＝**カードより深くは取らない**。実体はプラットフォームに残し、閲覧はリンク先で行う。

根拠: フロー型は保存の正当性（消失前の記録）が強く市場代替の害が小さい。ストック型は逆＝丸ごと保存はプラットフォームの客足・投稿者の engagement を代替してしまうため、公式に許された上限形（OGP カード・公式埋め込みプレイヤー）で受ける。テキスト主役コンテンツ（記事・小説）専用の UI（リーダー・本文抽出）は射程外。名乗りは一級レコード（投稿）の種類を指し、収蔵範囲の寛容さとは独立（前例＝Zotero: 文献管理を名乗りつつ添付は任意ファイル）＝段2を任意ファイルへ広げても名乗り・訴求は「SNS 投稿のライブラリ」のまま。「なんでも入る」は訴求の見出しにしない（機能リストの一行に置く）。
