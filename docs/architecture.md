...
- 投稿者ビュー（コンテンツ上部の「投稿 / 投稿者」モード切替トグルで投稿グリッド⇄投稿者グリッドを切替＝`browseMode`・前例 Bluesky/Xのプロフィールタブ）。投稿者カードはサイドカーの著者情報を `buildUsers()` が `platform:userId` でグルーピングして導出（追加のAPI取得なし）＝アバター（`avatarFile`・無ければモノグラム）＋投稿者名＋@ユーザー名＋PFドット＋投稿数、投稿数順ソート。**クリック=インスペクタ**（投稿者プロフィール＝名前/PF/投稿数、フォロワー/登録日は公開APIに有れば）、**ダブルクリック=投稿モードでその投稿者の `user` フィルタ**。検索ボックスは投稿者名/@で絞り込み（名前もハンドルも無いレコードは投稿者グリッドから除外）。投稿者モード中はフィルタ/ソート/密度/クエリビルダー/タグ付け/ワークスペースを隠し検索だけ残す。**残**: 投稿者グリッドのアバターは既存ライブラリが未backfillだと出ない（新規キャプチャ/`backfill-metadata.js --avatars` で順次）。投稿者へのタグ付け/フォルダ・SNS横断の名寄せ・投稿者モード専用サイドバーは未実装（BACKLOG）。
