...
2. **画像・メディアファイル＝実体保存**: ローカル取込・要素キャプチャ等。画像ライブラリの文法内で受ける。
...
根拠: フロー型は保存の正当性（消失前の記録）が強く市場代替の害が小さい。ストック型は逆＝丸ごと保存はプラットフォームの客足・投稿者の engagement を代替してしまうため、公式に許された上限形（OGP カード・公式埋め込みプレイヤー）で受ける。テキスト主役コンテンツ（記事・小説）専用の UI（リーダー・本文抽出）は射程外。
