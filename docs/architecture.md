...
- `protocol.mts` — **拡張⇄ホストのメッセージ契約の唯一の宣言**（#400）。リクエスト6種（`save`/`savePost`/`saveDragged`/`query`/`log`/`ping`）・応答・エラーコード・captureId の形（保存フォルダから出られない規則）・リクエストid の折り返し規則・プロトコル版番号（#205 が比較する値。互換分岐はここに書かない）を持ち、`parseHostFrame` が受信フレームを型の付いたリクエストか返答すべき失敗に変える。**拡張側がこのファイルを import する**（WXT/Vite がバンドル時に取り込む＝配布される拡張にこのディレクトリへの実行時依存は残らない）＝逆向きに置けない理由は、`native-host/` が `app/` 抜きで配られる別デリバラブルだから（electron-builder が生のまま extraResource で運ぶ）。**ブラウザのバンドルに入る唯一のファイル**なので node 組み込みに触れてはならず、`post-record.mts` / `raw-payload.mts` からの import は型だけ
