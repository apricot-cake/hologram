...
- `src/main/lib-model-registry.ts`＋`lib-model-fetch.ts`＋`lib-model-manager.ts` — モデルマネージャ（#832・[ADR 0026](decisions/0026-local-ml-foundation-boundaries.md)）。レジストリ（`{id, rev, files[{path, sha256, bytes}], licenseNote}`）が真実源で、`rev` は HF のコミットハッシュ固定。置き場は `~/.hologram/models/<org>/<name>@<rev>/`＝**端末資産なので保存フォルダの外**（バックアップ #233 とエクスポート #57 は保存フォルダしか歩かない）。取得だけ自前なのは SHA-256 検証を差し込むためと、通信を同意ゲートの内側へ1本化するため。`.part` へ書いて Range で再開し、ハッシュが合った時だけ rename で確定する。rev が上がっても自動更新しない（`installedRev` で知らせるだけ）。THIRD-PARTY-NOTICES.md との同期は `scripts/model-registry.test.ts`
- `src/main/lib-derived-db.ts` — 派生データストア（#833・[ADR 0026](decisions/0026-local-ml-foundation-boundaries.md)）。解析結果は `~/.hologram/derived.db`＝`hologram.db` とは**別ファイル**で、真実源（`posts` 系）には1行も書かない。**再構築できる側なので復旧規則が違う**＝`quick_check` が通らなければ捨てて作り直す（`hologram.db` なら `DatabaseCorruptError`）。キーは `captureId`＋`assetRef`（`image`/`video`/`file`/`media[seq]`）＋`segment`、行に `modelId`/`modelRev` を刻む（モデルを使わないジョブは両方 null＝出自ラベルを付けない根拠）。本基盤が持つのは全ジョブ種共有の `derived_progress` 1本だけで、機能テーブルは各機能 Issue が足す。完全削除との連動は `purgeDerivedForCapture`（ゴミ箱の間は残す）
