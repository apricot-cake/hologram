...
- 投稿者ビュー（コンテンツ上部の「投稿 / 投稿者」モード切替トグルで投稿グリッド⇄投稿者グリッドを切替＝`browseMode`・前例 Bluesky/Xのプロフィールタブ）。投稿者カードはサイドカーの著者情報を `buildUsers()` が `platform:userId` でグルーピングして導出（追加のAPI取得なし）＝アバター（`avatarFile`・無ければモノグラム）＋投稿者名＋@ユーザー名＋PFドット＋投稿数、投稿数順ソート。**クリック=インスペクタ**（投稿者プロフィール＝アバター/名前/PF/投稿数、フォロワー/登録日は公開APIに有れば＋**最近の作品サムネ6枚**＝その投稿者の投稿を `groupRecords` で新しい順にグループ化しリード画像を表示、サムネクリックでギャラリー）、**ダブルクリック=投稿モードでその投稿者の `user` フィルタ**。検索ボックスは投稿者名/@で絞り込み（名前もハンドルも無いレコードは投稿者グリッドから除外）。投稿者モード中は投稿専用UI（フィルタ/密度/クエリビルダー/タグ付け/ワークスペース）を隠し、**投稿者向けの並び順（投稿数/名前/最新投稿＝`latest` は buildUsers が最大日付で保持）＋プラットフォーム絞り込みチップ（存在するPFのみ）＋お気に入り絞り＋検索**を残す。**お気に入り**＝カード右上/インスペクタの★トグル（GitHubスター/iOSハートの語彙・accent塗り）。`platform:userId` を `poster-favorites.json`（`{keys:[...]}`・ungroupedと同形＝同じ export/backup/union-merge 経路）に永続化。**名前付きフォルダ**＝投稿者を複数の名前付きフォルダに整理（お気に入りの上位概念）。サイドバーで作成/削除/リネーム(dblclick)/クリックで絞り込み、割当はインスペクタのフォルダチップ（トグル＋＋で新規）。`poster-folders.json`（`{folders:[{id,name,items:[posterKey]}]}`・folders.jsonと同形＝import は既存 `mergeFolders` 再利用）。`get/set-poster-favorites`・`get/set-poster-folders` IPC。両ファイルとも `INTERNAL_FILES`（fs.watch無視）/clear-all保持/ORG_MERGE に登録済み。**残**: 投稿者グリッドのアバターは既存ライブラリが未backfillだと出ない（新規キャプチャ/`backfill-metadata.js --avatars` で順次）。投稿者へのタグ付け/フォルダ・SNS横断の名寄せ・投稿者モード専用サイドバーは未実装（BACKLOG）。
