...
## 実装状況（2026-06-20 実装セッション・全20件 着手済み）
- **完了（20件）**: C1, C3 ／ A1–A6 ／ B1（配置部）, B2, B3, B4, B5, B6, B7（B7 は A1 で同時解消）／ D1, D2 ／ E1, E2, E3。各コミット参照（fix/chore/refactor/docs(viewer)）。実機 CDP で初期化健全・フォルダストアの非破壊ロジック検証・各フライアウト/コンテキストメニューの配置/開閉まで確認済み。
- **B1 の内訳**: 配置クランプの集約は完了＝9ポップ中8つを2ヘルパへ（`clampIntoView`＝カーソル配置の5メニュー〔クエリビルダ/フォルダ/カード/種別/タブ〕、`placeFlyout`＝右アンカーの3つ〔サイドバーフィルタ flyout/日付/エンゲージ popover〕）。cs-pop と qb-help はボタン下開きドロップダウンの別パターンで少数のため据え置き。
- **B1 の意図的見送り（残さない＝やらない判断）**: 外側クリック/Esc の **document リスナー1組集約は不採用**。各ポップの閉じ処理は個別調整された挙動（qf の複雑な除外条件・kindMenu の `stopPropagation` で capture hider を生存・C3 が依存する capture/bubble の Esc 順序）を持ち、1行×8の重複削減より退行リスクが上回るため。重複はこのコストなら許容する。
