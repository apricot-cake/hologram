...
**状態: 設計確定**（DQ1–DQ8 全確定・2026-06-20）。実装は設計確定後に手で進める。

## 背景

BACKLOG「UX再設計の指摘（2026-06-20）」のタグ付けUX＋ナビ/検索を設計確定する作業。2つの作業を実施:

- **A=設計**（現状調査→複数案→判定→統合→批評）。批評の総合判定は needs-revision（下の「実装前に解く論点」で解消）。
- **B=viewer.js 健全性レビュー**（5レンズ→敵対的裏取り→優先度化）。確定27件→17項目（P0=1/P1=6/P2=10）。
...
全文は [docs/reviews/viewer-js-review-2026-06-20.md](reviews/viewer-js-review-2026-06-20.md)。以下は確定（敵対的裏取り済み）の高優先項目:
