...
### DQ7 検索の通常/あいまい切替の発見性 — 確定: **常設セグメント＋検索欄"直下"の独立行＋投稿者もあいまい対応**
- **形状**: **常設セグメント「通常｜あいまい」**（macOS Finder/Spotlight/Mail のスコープバー語彙＝凹トラック+浮きつまみ）。両方式を常時ラベル表示し、状態（つまみ位置）と切替手段を同時に可視化。既存 `.view-toggle`/`.vt-thumb`（密度トグル）部品を流用。
- **配置（確定）**: `.search-wrap` の**中**（入力右隣の単一ボタン）をやめ、**検索入力の直下に独立した行**として置く（検索 `.sb-section` 内・input の下）。入力欄は全幅維持。理由＝2ラベル＋つまみに幅が要る／前例 Finder・Spotlight・Mail の**スコープバーは検索フィールドの直下**に出る／A案の「検索欄直下」とも一致。**現状 `#searchModeBtn`（wrap内の単一トグルボタン・[index.html:1704]）は撤去**してセグメントへ置換。
- **設置面**: 検索欄は既にサイドバー（Mica接地面）にあり glass-on-glass 非該当。`.vt-thumb` は接地面の glass-relief ビーズで規範通り。
- **投稿者モード（旧・技術論点5の解消）— 確定: あいまい対応を投稿者の絞り込みにも展開**。`filteredPosters()`（[viewer.js:4545]）の `name.includes(q)` を `corpusSearch.compile(q)` 化し、`displayName`/`screenName` をあいまい一致に。→ **セグメントは全モードで意味を持ち、モード依存の無効化/非表示の特別扱いが不要**（実装シンプル・一貫）。短語は errBudget により実質 exact なので名前検索のノイズは限定的。
- **フライアウト `qfModeBtn`**: 既に `corpusSearch.onChange(syncQfMode)` 購読済み（commit `530b5ae`・stale修正済み）。任意で同セグメント部品へ寄せる（polish・別途）。
...
4. **DQ7**: L51 を「検索方式はセグメンテッドコントロール（スコープバー）＝**検索入力の直下に独立行**で常設。前例 Finder/Spotlight/Mail のスコープバー（フィールド直下）」に更新。単一トグルボタンは退役。
