...
> **【更新 2026-06-15】タブ本体は実装済み。「戻る/進む（履歴・Alt+←→・`viewHistory`/`histIdx`/`history`）」は撤去済み**＝本書の履歴関連の記述（共存・タブごとの history 保持・switchTab の _history 退避/復元・履歴テスト等）は**無効**。タブの state 保持はスクロール位置（`_scrollTop`・セッション内）に置き換わっている。
