...
// format.ts のロジック単体テスト。format.ts は real ES module（named exports）
// なので動的 import() で読み込む。件数の短縮（formatCount）・日付整形
...
// CI ロケール差でのフレーク化を避ける。
//
//   node scripts/test-format-unit.cts

const path = require('node:path');
const { pathToFileURL } = require('node:url');

async function main() {
  const F = await import(pathToFileURL(path.join(__dirname, '..', 'app', 'src', 'renderer', 'src', 'services', 'format.ts')).href);
