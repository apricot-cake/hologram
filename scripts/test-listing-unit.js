...
// Pure unit test for listing.js (window.corpusListing) — the 7th viewer.js
// extraction slice. listing.js dual-exports via CommonJS, so require it
// directly and drive getFilteredPosts (content gate → query tree → sticky
// merge → sort), namedPosters/filteredPosters, the collection derivations
// (legacy-q folding / dynamic matching / per-pass record cache / thumbs /
// counts / cond chips / filteredCollections) and cloneTree with stub deps.
//
//   node scripts/test-listing-unit.js
...
let tree = { kind: 'group', op: 'and', neg: false, children: [] };
let sort = 'none';
let search = '';
...
let posterTree = { kind: 'group', op: 'and', neg: false, children: [] };
let posterEval = () => true;
