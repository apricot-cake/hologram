...
// library database (<saveFolder>/hologram.db, #176/ADR 0024) both go there —
...
app.whenReady().then(async () => {
  ...
  const handle = openDatabase(path.join(folder, 'hologram.db')); // #176: inside the save folder now, not configDir (ADR 0024)
