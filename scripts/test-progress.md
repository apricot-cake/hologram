...
| A-1l | - | |
| A-1m | - | |
| A-1n | SKIP | ★E2E自動。/photo/1 の返信欄に画像なし → graceful skip（ライトボックス本体のみの投稿では検証不能・手動機会待ち） |
| A-1o | OK | ★E2E自動（初回実行）。x.com/jack のアバターをドラッグ → drop zone 非表示・保存なし（捏造防止 PASS）。2回目以降はXのセッション保護でプロファイルページ自体がブロックされ不安定 |
