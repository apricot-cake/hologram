...
async function recentMisskeyUrl() {
  const r = await fetch('https://misskey.io/api/notes/global-timeline', {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ limit: 60 })
  });
  const j = await r.json();
...
(async () => {
  ...
  try {
    ...
    if (burl) {
      ...
      show('Bluesky (' + burl + ')', b);
      if (!(b.userId && b.userId.startsWith('did:') && b.screenName && mediaOk(b))) { pass = false; console.log('  Bluesky FAIL'); }
  ...
  try {
    ...
    if (murl) {
      ...
      show('Misskey (' + murl + ')', m);
      if (!(m.screenName && m.date && mediaOk(m))) { pass = false; console.log('  Misskey FAIL'); }
