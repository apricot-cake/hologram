...
//
//   node scripts/test-tag-types.js
...
// --- mergeTagTypes (pure) -----------------------------------------------------
{
  ...
  const l = mergeTagTypes(
    { types: {}, labels: { work: '作品' } },
    { types: {}, labels: { work: 'シリーズ', character: '話数' } }
  );
...
(async () => {
  ...
  fs.writeFileSync(path.join(dest, 'tag-types.json'),
    JSON.stringify({ types: { アリス: 'character', ブルアカ: 'work' } }), 'utf8');

  // Incoming ZIP: adds アロナ=character, and tries to flip アリス→work (must lose).
  const zip = new JSZip();
  zip.file('library/cap1.jpg', Buffer.from('JPEGDATA1'));
  zip.file('library/tag-types.json',
    JSON.stringify({ types: { アロナ: 'character', アリス: 'work' } }));
