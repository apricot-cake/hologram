...
//
//   node scripts/test-collections-merge.js
...
(async () => {
  // --- mergeCollections (pure) ---
  {
    const cur = {
      ...
      activeId: 'c-x', posterWorkspace: ['p1'],
    };
    const inc = {
      collections: [
      ...
      activeId: 'f-2', posterWorkspace: ['p2'],
    };
    ...
    assert.ok(m.collections.find((c) => c.id === 'f-2'), 'new incoming id added');
    ...
    const m2 = mergeCollections({ collections: [], activeId: 'gone', posterWorkspace: [] },
      { collections: [{ id: 'f-3', name: 'X', items: [] }], activeId: 'nope', posterWorkspace: [] });
    assert.strictEqual(m2.activeId, null, 'invalid activeId → null');
    // incoming activeId adopted only if local is null and it exists
    const m3 = mergeCollections({ collections: [], activeId: null, posterWorkspace: [] },
      { collections: [{ id: 'f-9', name: 'Y', items: [] }], activeId: 'f-9', posterWorkspace: [] });
  ...
  // --- importCompleteZip: legacy folders.json folds into an existing collections.json ---
  {
    ...
    fs.writeFileSync(path.join(dest, 'collections.json'), JSON.stringify({
      collections: [{ id: 'c-local', name: 'Local WS', kind: 'static', created: null, items: ['x'] }],
      activeId: 'c-local', posterWorkspace: ['p-local'],
    }));
    const zip = new JSZip();
    ...
    assert.ok(col.collections.find((c) => c.id === 'c-local'), 'local collection kept');
    assert.ok(col.collections.find((c) => c.id === 'f-imp'), 'imported folder folded in as collection');
  ...
  // --- importCompleteZip: a collections.json ZIP merges into the local one ---
  {
    ...
    fs.mkdirSync(dest, { recursive: true });
    fs.writeFileSync(path.join(dest, 'collections.json'), JSON.stringify({
      collections: [{ id: 'c1', name: 'L', kind: 'static', created: null, items: ['a'] }], activeId: null, posterWorkspace: [],
    }));
    const zip = new JSZip();
    zip.file('library/collections.json', JSON.stringify({
      collections: [{ id: 'c1', name: 'R', kind: 'static', created: null, items: ['b'] }, { id: 'c2', name: 'New', kind: 'static', created: null, items: ['c'] }],
      activeId: 'c2', posterWorkspace: ['pp'],
    }));
    const buf = await zip.generateAsync({ type: 'nodebuffer' });
    await importCompleteZip(JSZip, dest, buf);
    ...
    assert.ok(col.collections.find((c) => c.id === 'c2'), 'new collection added');
...
})().catch((e) => { console.error('MERGE_TEST_FAIL:', e && e.message ? e.message : e); process.exit(1); });
