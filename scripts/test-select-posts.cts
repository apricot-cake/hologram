...
async function selectX() {
  ...
  row(
    'A-1p',
    'X クリック（ライトボックス）★修正検証',
    obama ? 'https://x.com/BarackObama/status/266031293945503744/photo/1' : null,
    'ライトボックスを開いて Alt+S → 拡大中の画像にカーソルを合わせてクリック',
    '画像にハイライト枠が出て（枠は画像の輪郭であってモーダル全体ではない）、そのままクリックで /photo/N を落としたパーマリンクの投稿として保存される。背景・閉じるボタン・返信欄の画像では枠が出ない（返信欄の画像は返信自身の枠）',
  );
}
