...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'postsnap-render-'));
const configDir = path.join(tmp, 'PostSnap'); // paths.js (win32) => %APPDATA%/PostSnap
const saveFolder = path.join(tmp, 'saves');
fs.mkdirSync(configDir, { recursive: true });
fs.mkdirSync(saveFolder, { recursive: true });
fs.writeFileSync(path.join(configDir, 'config.json'),
  JSON.stringify({ saveFolder, extensionId: 'testextensionidabcdefghijklmnop' }));
...
fs.writeFileSync(path.join(saveFolder, `${captureId}.json`), JSON.stringify({
  captureId,
  image: `${captureId}.jpg`,
  ...
  displayName: 'てすと太郎',
  screenName: 'testuser',
  likes: 24853, reposts: 3210, replies: 142,
