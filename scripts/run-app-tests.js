'use strict';
// App-harness aggregator: runs every scripts/test-app-*.js in sequence and exits
...
// boot) — it is deliberately NOT part of npm test (run-tests.js = pure units). Run it
...
//
// Run all:     node scripts/run-app-tests.js
// Run a subset: node scripts/run-app-tests.js tabs search   (suffix match)
...
const all = fs
  ...
  .filter((f) => /^test-app-.*\.js$/.test(f))
  .sort();
const args = process.argv.slice(2);
const picked = args.length ? all.filter((f) => args.some((a) => f === a || f === `test-app-${a}.js`)) : all;
if (!picked.length) {
  console.error(`no matching test-app-*.js (have: ${all.join(', ')})`);
  process.exit(2);
}
