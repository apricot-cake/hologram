...
describe('マイグレーションが通り、テーブルが揃う', () => {
  ...
  test('user_version は 27（v1 DDL ＋ #178 add-post-cw-sensitive・#188 add-post-series-fields・#180 add-post-quoted-refs・#162 add-media-max-dims・#290 add-post-custom-emojis・#36 rename-description-to-memo・#23 add-poster-aliases・#179 add-post-poll・#236 add-post-file までの追加26本）', () => {
    const { sqlite } = openDatabase(mkdb());
...
describe('既存 v1 データベースの開き直しは no-op', () => {
  ...
  test('マイグレーションを再実行しない', () => {
    expect(second.sqlite.pragma('user_version', { simple: true })).toBe(27);
  });
