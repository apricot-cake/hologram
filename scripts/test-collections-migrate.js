...
//
//   node scripts/test-collections-migrate.js
...
function runCase(captures, foldersJson) {
  return new Promise((resolve) => {
    ...
    for (const c of captures) {
      ...
      fs.writeFileSync(path.join(saveFolder, c.id + '.json'), JSON.stringify({
        captureId: c.id, image: c.id + '.jpg', url: 'https://www.pixiv.net/artworks/' + c.art,
        platform: 'pixiv', userId: c.userId, title: 'T' + c.id, displayName: 'D' + c.userId, screenName: c.userId,
        likes: 1, capturedAt: '2026-04-01T12:00:00Z', date: '2026-04-01T10:00:00Z', media: [], tags: [], hashtags: [], source: 'eagle-migration',
      }, null, 2));
    }
