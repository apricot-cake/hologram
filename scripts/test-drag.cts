...
//
//   node scripts/test-drag.cts

const fs = require('node:fs');
const os = require('node:os');
const path = require('node:path');

const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'hologram-drag-'));
...
(async () => {
  (global as any).fetch = async (_url, opts) => {
  ...
  (global as any).fetch = async () => ({ ok: true, headers: new Map([['content-type', 'text/html']]), arrayBuffer: async () => Buffer.from('x') });
  let threw = false;
  try {
