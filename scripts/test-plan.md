# Post Snap Test Plan

繰り返し利用可能なテストケース集。新機能追加時はケースを追加する。

> **Phase 1 アーキテクチャ**: キャプチャは EXIF / chrome.storage を使わず、Native Messaging ブリッジ経由で保存先フォルダに `<captureId>.jpg`（純JPEG）+ `<captureId>.json`（サイドカー）を書き出す。閲覧は Electron アプリ（`app/`）。下記 **C/D/E/F（ビューア・設定・エクスポート/インポート・削除）は旧 chrome.storage ビューア前提のため、Phase 2（Electron アプリへの全機能移植）で再定義する**。

## 事前準備（毎回）

- Electron アプリ（`cd app && npm start`）で保存先フォルダと拡張IDを設定済みであること（初回起動で Native Messaging host が登録される）。
- 拡張とテスト対象ページをリロード済みであること（リロード検出: `python scripts/check-reload.py`）。

## テスト後の一括検証（毎回）

```
python scripts/verify-store.py --recent N
```

保存フォルダの `<id>.json` サイドカー（メタデータ）と `<id>.jpg`（ペア画像の存在）を、Bluesky / Misskey の公開APIと自動照合。

---

## 共通確認項目（全キャプチャテスト共通）

各キャプチャ後に以下を確認。verify-store.py が自動チェックする項目は [auto] と記載。

- [auto] サイドカー内の screenName, displayName, userId, text, date（API照合。X は screenName のみ）
- [auto] サイドカー内の likes, reposts, replies（API照合・件数は変動許容）
- [auto] `<captureId>.jpg` と `<captureId>.json` のペアが保存先フォルダに生成
- [ ] サイドカー内の captureId, capturedAt, mediaType, lang, isReply, isQuote, isThread（参考表示・手動確認）
- [ ] capture-log.txt（開発用ログ・Downloads）: 各フィールドが期待値と一致

---

## A. プラットフォーム × ページ種別

各セルで1回保存 → verify-exif.py + ページ照合で検証。

### 検証手順

1. 対象ページで Alt+S → 投稿をクリックして保存
2. `python scripts/verify-exif.py` で自動検証
3. X の場合は追加でページ実データと照合（パブリックAPIがないため）

### A-1. X (Twitter)

| # | ページ | 選ぶべき投稿 | 注目点 |
|---|--------|-------------|--------|
| A-1a | TL (x.com/home) | いいね・RT・返信が全て1以上 | エンゲージメント取得、カンマ区切り変換（1,234→1234） |
| A-1b | 個別 (x.com/{user}/status/{id}) | A-1a と別の投稿 | 個別ページのDOM構造で permalink・userId が取れるか |
| A-1c | プロフィール (x.com/{user}) | 自分以外のユーザーページの投稿 | フォローボタンからの userId フォールバック |
| A-1d | TL上のRT | リツイート | userId が null（仕様）。screenName は元投稿者 |
| A-1e | TL上の引用 | 引用ツイート | 引用した側の article が対象になるか |
| A-1f | 個別: リプライ | 他ユーザーへの返信 | isReply=true, isThread=null |
| A-1g | 個別: 引用 | 引用ツイート | isQuote=true |
| A-1h | 個別: セルフリプ | 自分への返信 | isThread=true, isReply=null |
| A-1i | 個別: 動画投稿 | 動画付き | mediaType=video |
| A-1j | 個別: 英語投稿 | 英語テキスト | lang=en |

**X 固有の確認**: ページ実データと照合（displayName, text, date, likes, reposts, replies, userId）

### A-2. Bluesky

| # | ページ | 選ぶべき投稿 | 注目点 |
|---|--------|-------------|--------|
| A-2a | TL (bsky.app) | いいね・リポスト・返信全て1以上 | セレクタ `feedItem-by-{handle}` |
| A-2b | 個別 (bsky.app/profile/{h}/post/{id}) | A-2a と別の投稿 | `postThreadItem-by-{handle}`。API enrichment 確認 |
| A-2c | プロフィール (bsky.app/profile/{handle}) | 他ユーザーの投稿 | プロフィールページでの検出 |
| A-2d | TL上のリポスト | リポストされた投稿 | 元投稿のメタデータが取れるか |
| A-2e | 個別: リプライ | 返信投稿 | isReply=true |
| A-2f | 個別: 引用 | 引用投稿 | isQuote=true |

**Bluesky 固有の確認**: userId (DID) が取得されている、API経由でエンゲージメント取得

### A-3. Misskey

| # | ページ | 選ぶべき投稿 | 注目点 |
|---|--------|-------------|--------|
| A-3a | TL ({instance}/) | リアクション・リノート・返信全て1以上 | `div[tabindex="0"]` + article で検出 |
| A-3b | 個別 ({instance}/notes/{id}) | A-3a と別のノート | `location.href` フォールバック |
| A-3c | プロフィール ({instance}/@{user}) | 他ユーザーのノート | プロフィール抽出 |
| A-3d | TL上のリノート | リノートされたノート | renoteCount がAPI経由で取得 |
| A-3e | 個別: リプライ | 返信ノート | isReply=true |
| A-3f | 個別: 引用 | 引用リノート | isQuote=true |

**Misskey 固有の確認**: API補完（screenName, displayName, userId, likes=リアクション合計）

---

## B. UI / インタラクション

| # | 操作 | 確認項目 |
|---|------|---------|
| B-1 | Alt+S で起動 | バナー表示（日本語 or 英語）。「右クリックで設定」を含む |
| B-2 | 投稿にホバー | 青い枠。投稿間でスムーズにアニメーション |
| B-3 | 投稿外にマウス | 枠が非表示 |
| B-4 | 投稿をクリック | 保存中（グレー）→ 成功（緑）→ 1.5秒後に消える |
| B-5 | Escキー | キャプチャモード終了。バナー・枠が消える |
| B-6 | 右クリック | ビューアページが開く。キャプチャモード終了 |
| B-7 | 画面外の投稿をクリック | 自動スクロール → キャプチャ → スクロール位置復元 |
| B-8 | Alt+S 連打（二重起動） | 最初のセッションがクリーンアップされエラーにならない |

※ B-1〜B-4 は A のテスト実行中に自然に確認できる。別途やる必要があるのは B-5〜B-8。

---

## C. ビューア

### C-1. 表示

| # | 操作 | 確認項目 |
|---|------|---------|
| C-1a | ビューアを開く | カード形式で表示。サムネイル、バッジ、ユーザー名、テキスト抜粋、エンゲージメント、日時、タグ |
| C-1b | 投稿がない場合 | 空状態表示 |
| C-1c | 投稿カードをクリック | 元の投稿URLが新しいタブで開く |
| C-1d | 長いテキストをクリック | 全文表示。再クリックで折りたたみ |
| C-1e | 虫眼鏡ボタン | ライトボックス表示。クリックで閉じる |

### C-2. 検索・ソート・フィルタ

| # | 操作 | 確認項目 |
|---|------|---------|
| C-2a | テキスト検索 | 投稿テキスト / screenName / displayName で絞り込み |
| C-2b | 検索結果なし | 空状態表示 |
| C-2c | ソート | 新しい順 / 古い順 / いいね順 / リポスト順 / 返信順 |
| C-2d | プラットフォームフィルタ | X / Bluesky / Misskey チップで切替 |
| C-2e | 投稿タイプフィルタ | ポスト / リプライ / 引用 / セルフリプ |
| C-2f | メディアフィルタ | 画像 / 動画 / GIF |
| C-2g | 日付範囲フィルタ | 投稿日 / キャプチャ日切替 + from/to |
| C-2h | エンゲージメントフィルタ | 種類選択 + 以上/以下 + 値 |
| C-2i | タグフィルタ | サイドバーのタグチップで切替 |
| C-2j | 複合フィルタ | 同カテゴリOR、異カテゴリAND |
| C-2k | 件数表示 | フィルタ適用後の件数 |
| C-2l | マルチセレクト | 選択モード → チェック → 一括削除 |
| C-2m | タグ編集 | 編集ボタン → タグ追加/削除 → 保存 |

---

## D. 設定（ビューア内）

| # | 操作 | 確認項目 |
|---|------|---------|
| D-1 | フォルダ名を変更して保存 | 次回の保存先が変わる（Post Snap/配下） |
| D-2 | フォルダ名を空にして保存 | デフォルト「Post Snap」に戻る |
| D-3 | 不正なフォルダ名（「..」「/」「\」） | エラー表示。保存されない |
| D-4 | saveAs ON/OFF | ON: 「名前を付けて保存」ダイアログ。OFF: サイレント保存 |
| D-5 | ショートカットリンク | chrome://extensions/shortcuts が開く |
| D-6 | ビルドハッシュ表示 | `Build: {hash}` が check-reload.py と一致 |

---

## E. エクスポート / インポート

| # | 操作 | 確認項目 |
|---|------|---------|
| E-1 | ZIP エクスポート | `post-snap-export-YYYY-MM-DD.zip`。`images/` に JPEG、`metadata.json` にメタデータ配列 |
| E-2 | HTML エクスポート | `post-snap-export-YYYY-MM-DD.html`。別ブラウザで開いて検索・ソート・フィルタ動作 |
| E-3 | データなしでエクスポート | トースト表示 |
| E-4 | 画像からインポート | JPEG の EXIF XPComment JSON → storage に復元。複数ファイル一括可 |
| E-5 | HTML からインポート | エクスポート HTML の JSON → storage に復元 |
| E-6 | 重複スキップ | 同じ URL の投稿は既存データがある場合スキップ（画像・HTML 共通） |
| E-7 | 無効ファイル | EXIF JSON なし JPEG / `postSnapData` なし HTML → スキップ or エラー |
| E-8 | 復元フロー | storage クリア → 画像 or HTML インポート → データ完全復元 |

---

## F. データ削除

| # | 操作 | 確認項目 |
|---|------|---------|
| F-1 | 「全データを削除」 | 確認ダイアログ → 実行で posts 削除、一覧空。キャンセルで変化なし |
| F-2 | 削除後の確認 | ダウンロード済み画像ファイルは影響なし |

---

## G. セキュリティ

| # | 条件 | 確認項目 |
|---|------|---------|
| G-1 | 非対応サイトで Alt+S | getSiteConfig が null → 何も起きない |
