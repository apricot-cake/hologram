...
- WXT 開発中は `npm run dev:ext` を継続しておく。ソース保存後の拡張再読み込みは WXT に任せ、対象ページだけ再読込する。
...
| A-1p | クリック: ライトボックス | /photo/N 表示中に拡大表示中の画像 | 画像にハイライト枠が出る（モーダル全体ではない）。保存されるのは /photo/N を落としたパーマリンクの投稿。背景・閉じるボタンでは枠が出ない |
