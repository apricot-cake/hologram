...
// facets.ts のロジック単体テスト。facets.ts は real ES module（named exports）なので
// 動的 import() で読み込み、facetCounts（バケット集計）と qfValues（15カテゴリの
// フライアウト行モデル）をスタブ deps 注入で直接検証する。
//
//   node scripts/test-facets-unit.cts
