...
//
//   node scripts/test-select-posts.js          (needs network)
//
// Flow: run this → open each URL → Alt+S + click (or drag) → the watcher
// (scripts/test-watch-verify.js) auto-verifies every capture as it lands.
...
(async () => {
  ...
  console.log(`生成: ${new Date().toISOString()}　手順: URLを開く → Alt+S → アクション。検証は test-watch-verify.js が自動。\n`);
