...
const evalJs = `(async () => {
  ...
  r.effBefore = effOf(await window.hologram.getPosterTags(), 'x:@u0', '東方');
  await window.hologram.removeTagParent(${remiliaId}, ${touhouId});
  r.effAfter = effOf(await window.hologram.getPosterTags(), 'x:@u0', '東方');
  return r;
})()`;
