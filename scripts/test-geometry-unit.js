...
// geometry.js（window.corpusGeometry）のロジック単体テスト。CommonJS でも export
// するので直接 require し、列数計算（colsFor/sizeFor/minColsFor）・スライダー
// トラック導出（sliderTrack/trackCols＝反転規約）・サムネ幅の60px量子化（thumbW）
// を検証する。旧 viewer.js の pColsFor/tileColsFor（重複実装）が単一所有へ
// 統合されたスライスの回帰ガード。
//
//   node scripts/test-geometry-unit.js
