...
//
//   node scripts/test-archive-zipbomb.js
...
const archive = require('../app/lib-archive.js');
const {
  importCompleteZip, writeEntryStreamed, ZipLimitError,
  MAX_ZIP_ENTRIES, MAX_ZIP_ENTRY_BYTES, MAX_ZIP_TOTAL_BYTES
} = archive;
...
function ForgingJSZip(sizeFor) {
  const Wrap = function () { return new JSZip(); };
  Wrap.loadAsync = async (buf) => {
    const z = await JSZip.loadAsync(buf);
    let i = 0;
    z.forEach((rel, e) => { if (!e.dir && e._data) e._data.uncompressedSize = sizeFor(rel, i++); });
    return z;
  };
  return Wrap;
}
...
(async () => {
  ...
  // the TOTAL guard can fire). 80 entries × ~0.99 GiB ≈ 79 GiB > 64 GiB.
  {
    ...
    await expectReject(importCompleteZip(ForgingJSZip(() => each), dest, small.buf), 'total-bomb');
  ...
  // this drives importCompleteZip's count tally exactly as a real bomb would.
  {
    ...
    const SyntheticJSZip = function () { return new JSZip(); };
  ...
  // (d) A single entry over the per-entry cap.
  {
    ...
    await expectReject(importCompleteZip(ForgingJSZip((rel, i) => (i === 0 ? oversize : 4)), dest, small.buf), 'entry-bomb');
  ...
  // central directory defense), and leaves no readable partial file behind.
  {
    ...
    let threw = false;
    try { await writeEntryStreamed(entry, tmp, 64 * 1024); }   // 64 KiB budget < 256 KiB payload
    catch (e) { threw = true; assert.ok(e instanceof ZipLimitError, 'stream-cap: expected ZipLimitError'); }
...
})().catch((e) => { console.error('FAIL test-archive-zipbomb:', e && e.message ? e.message : e); process.exit(1); });
