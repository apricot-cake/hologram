...
describe('操作系コマンド', () => {
  test('空クエリでも全部出る（まず何ができるかが読める）', () => {
    expect(titlesOf(R.queryEntries('', PALETTE), 'command')).toEqual(['cmdOpenSettings', 'cmdNewTab', 'cmdClearFilters', 'cmdViewGallery', 'cmdViewList', 'cmdBrowsePosts', 'cmdBrowsePosters']);
  });
