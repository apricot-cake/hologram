...
// cooc.ts のロジック単体テスト。cooc.ts は real ES module（named exports）なので
// 動的 import() で読み込み、charCandidatesFor（強＝作品→キャラ）・worksCooccurringWith
...
async function main() {
  const C = await import(pathToFileURL(path.join(__dirname, '..', 'app', 'src', 'renderer', 'src', 'services', 'cooc.ts')).href);
