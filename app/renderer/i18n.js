...
(function () {
  const MESSAGES = {
    ja: {
      ...
      themeTitle: '外観',
      themeDark: 'ダークテーマ',
      hintTheme: '暗い環境で長時間眺めるときに。設定はすぐ反映され、次回も維持されます。',
      ...
      backupContentMeta: 'メタデータ込み（画像＋JSON＝あとで復元できる）',
      backupContentMedia: 'メディアのみ（画像/動画だけ・JSONなし）',
      backupScheduleTitle: '自動エクスポートの契機',
      backupOnStart: 'アプリ起動時に毎回',
      backupInterval: '一定間隔：',
      backupIntervalUnit: '時間ごと',
      backupOnChange: '変更時（数分まとめてから）',
    ...
    en: {
      ...
      hintTheme: 'For long viewing sessions in dim light. Applies immediately and persists.',
      ...
      backupContentMeta: 'With metadata (images + JSON, restorable)',
      backupContentMedia: 'Media only (images/videos, no JSON)',
      backupScheduleTitle: 'Auto-export triggers',
      backupOnStart: 'On every app start',
      backupInterval: 'Every',
      backupIntervalUnit: 'hours',
      backupOnChange: 'On changes (batched after a few min)',
