...
(function () {
  const MESSAGES = {
    ja: {
      ...
      detailUpload: 'アップロードで検索',
      sauceTitle: '類似画像検索',
      sauceSearching: '検索中…',
      sauceNoKey: 'アップロード検索には SauceNAO の API キーが必要です。設定 → 類似画像検索 で登録してください（無料）。',
      sauceNoHits: '一致する画像は見つかりませんでした。',
      sauceErr: '検索に失敗しました',
      sauceOpenSettings: '設定を開く',
      settingsSauceTitle: '類似画像検索',
      hintSaucenao: 'SauceNAO の API キー（saucenao.com で無料登録）。インスペクタの「アップロードで検索」で保存画像を逆引きします。公開元URLがある投稿はキー不要で SauceNAO/ascii2d を開けます。',
      imagesCount: '$1 枚',
      ...
      qbHelp4: '箱と箱のつなぎ方は、間のプルダウン（かつ⇄または）で切り替え。',
    ...
    en: {
      ...
      detailUpload: 'Search by upload',
      sauceTitle: 'Reverse image search',
      sauceSearching: 'Searching…',
      sauceNoKey: 'Upload search needs a SauceNAO API key. Add it in Settings → Reverse image search (free).',
      sauceNoHits: 'No matching images found.',
      sauceErr: 'Search failed',
      sauceOpenSettings: 'Open settings',
      settingsSauceTitle: 'Reverse image search',
      hintSaucenao: 'SauceNAO API key (free at saucenao.com). Powers "Search by upload" in the inspector to reverse-search a saved image. Posts with a public original can open SauceNAO/ascii2d without a key.',
      ...
      qbHelp4: 'The pulldown between the boxes switches how they combine (AND ⇄ OR).',
