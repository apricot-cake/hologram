...
// "pure logic → service" slice of the viewer decomposition (最終形B). Plain IIFE
// on window (like query.js / records.js / facets.js); loaded BEFORE viewer.js;
// touches no DOM. Runtime couplings are injected via makeUsers(deps) — reassigned
// viewer lets (allPosts / _allPostsGeneration) come in as getter functions, and
// corpusSearch as a getter too (buildSuggest reads its live fuzzy mode per call) —
// so this file loads under Node (scripts/test-users-unit.js). CommonJS-exported
// like records.js.
(function () {
  'use strict';
  ...
  const api = { makeUsers };
  if (typeof window !== 'undefined') window.corpusUsers = api;
