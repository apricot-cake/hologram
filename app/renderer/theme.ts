...
/* Theme: auto (follow the OS) / light / dark.
   ...
   Built standalone (islands/build.mjs, Vite lib IIFE → renderer/theme.js) instead
   of folding into the app.js bundle: it must run during <head> parse, before first
   paint, which app.js (loaded at the end of body) cannot guarantee — see the load-
   order comment in index.html. A browser can't type-strip .ts at runtime the way
   Node's main/native-host layers do, so this one file needs its own tiny build
   step (same reason preload.js stays .js — see tsconfig.main.json). */
(function () {
