// 共有検索ユーティリティ。検索方式（通常＝部分一致 / あいまい）を一元管理し、
// 投稿モード(viewer.js)・画像モード(image-viewer.js)の両方が参照する。
...
(function () {
  ...
  function normalize(s) {
    if (s == null) return '';
    let t = String(s);
    try { t = t.normalize('NFKC'); } catch (_e) { /* 古い環境向けフォールバック */ }
  ...
  function errBudget(len) { return len <= 2 ? 0 : len <= 4 ? 1 : 2; }
  ...
  function compile(query) {
    ...
    const terms = nq.split(/\s+/).filter(Boolean).map((t) => ({ t, k: errBudget(t.length) }));
  ...
  window.corpusSearch = {
    ...
    fuzzy(hay, query) { return compile(query)(hay); }
  };
})();
