(async () => {
  ...
  function resetAllFilters() {
    ...
    setSearchBoxValue(''); set('sbDateFrom', ''); set('sbDateTo', ''); set('sbEngMin', '');
    afterQueryChange();
    posterReturn = null;
  ...
  document.getElementById('emptyState').addEventListener('click', (e) => {
    ...
    if (btn.id === 'emptyResetBtn') {
      if (browseMode === 'posters') { setSearchBoxValue(''); renderPosters(); }
  ...
  function qfValues(cat) {
    ...
    switch (cat) {
      ...
      case 'poster-tag': {
        ...
        return posterFilterVocab().filter((t) => !tagKindOf(t))
          .map((t) => ({ v: t, l: t, on: posterQB.qHasValue('tag', t), count: cnt.get(t) || 0, facetDim: true }))
      ...
      case 'poster-character': {
        ...
        const cnt = facetCounts((u) => posterTagsOf(u.key), filteredPosters());
        return posterFilterVocab().filter((t) => tagKindOf(t) === kind)
          .map((t) => ({ v: t, l: t, on: posterQB.qHasValue('tag', t), kind, count: cnt.get(t) || 0, facetDim: true }))
          .sort((a, b) => b.count - a.count || a.l.localeCompare(b.l, 'ja'));
      }
      case 'poster-platform': {
        ...
        return [...present].sort((a, b) => { const ia = PF_ORDER.indexOf(a), ib = PF_ORDER.indexOf(b); return (ia < 0 ? 99 : ia) - (ib < 0 ? 99 : ib); })
          .map((v) => ({ v, l: PF_NAME[v] || v, on: posterQB.qHasValue('platform', v), count: cnt.get(v) || 0 }));
      }
      case 'poster-instance': {
        ...
        return [...hosts].map((h) => ({ v: h, l: h, on: posterQB.qHasValue('instance', h), count: cnt.get(h) || 0, facetDim: true }))
          .sort((a, b) => b.count - a.count || a.l.localeCompare(b.l));
      }
      case 'poster-folder': {
        const folders = pfStore.all();
        const cnt = facetCounts((u) => folders.filter((f) => f.items.includes(u.key)).map((f) => f.id), filteredPosters());
        return folders.map((f) => ({ v: f.id, l: f.name, on: posterQB.qHasValue('folder', f.id), count: cnt.get(f.id) || 0 }));
      }
      case 'work':
      case 'character': {
  ...
  const postQB = createQueryBuilder({
    ...
    onClearSearch: () => { setSearchBoxValue(''); afterQueryChange(); },
    ...
    onLeafMutated: (node) => { if (node === editingTextNode) { editingTextNode = null; setSearchBoxValue(''); } },
  ...
  const posterQB = createQueryBuilder({
    ...
    glyphOf: qcGlyph,
    getSearchVal: () => searchQuery(),
    onClearSearch: () => { setSearchBoxValue(''); renderPosters(); },
