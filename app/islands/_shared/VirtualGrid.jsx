...
export const useGridModel = () => useContext(ModelCtx);

export function VirtualGridHost({ model, cell }) {
...
export function wireGridIsland({ bridge, containerId, hostId, renderHost }) {
  ...
  const sync = () => {
    ...
    if (model) {
      if (!host.isConnected) document.getElementById(containerId).appendChild(host);
