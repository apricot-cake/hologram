...
// Row icons — copied verbatim from the pre-island HTML so the rendered SVG is identical
// (CSS sizes .sb-row-ic > svg). Full <svg> strings, not just paths, to keep per-icon
// attrs (e.g. the クリップ / 複数画像 icons carry their own width/height).
const ICON: Record<string, string> = {
  ...
  collection: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 20a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-7.9a2 2 0 0 1-1.69-.9L9.6 3.9A2 2 0 0 0 7.93 3H4a2 2 0 0 0-2 2v13a2 2 0 0 0 2 2Z"/></svg>',
...
const ARROW = '<svg width="11" height="11" viewBox="0 0 11 11" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3.5 1.5l4 4-4 4"/></svg>';
...
function Glyph({ className, svg }: { className: string; svg: string }) {
...
function Badge({ cat, badge }: { cat: string; badge: number }) {
  return (
...
function Row({ cat, label, badge, open, style }: { cat: string; label: string; badge: number; open: boolean; style?: CSSProperties }) {
  return (
