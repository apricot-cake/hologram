...
function AlertDialogOverlay({ className, ...props }: AlertDialogPrimitive.Backdrop.Props) {
  ...
  // it. Still below the z-[13500] popover portals. modal-backdrop-dim spares the titlebar
  // band so the OS window-control strip doesn't float (see dialog.tsx / globals.css).
  // Re-apply both on `shadcn add alert-dialog`.
  return <AlertDialogPrimitive.Backdrop data-slot="alert-dialog-overlay" className={cn('modal-backdrop-dim fixed inset-0 isolate z-[13100] duration-100 data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0', className)} {...props} />;
}
  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Popup
        ...
        data-size={size}
        className={cn(
          'font-sans group/alert-dialog-content fixed top-1/2 left-1/2 z-[13100] grid w-full -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl bg-popover p-4 text-popover-foreground ring-1 ring-foreground/10 duration-100 outline-none data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-sm data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95',
          className,
        )}
        {...props}
      />
