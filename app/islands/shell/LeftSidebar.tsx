...
import { all as folderAll, createFolder, isSavedSearch, load as folderLoad, onChange as folderOnChange, removeFolder, renameFolder, toast, updateFolder } from '../../renderer/folders.ts';
  ...
  return (
    <Collapsible render={<SidebarMenuItem />} open={isOpen} onOpenChange={(open) => ctx.setOpen(f.id, open)}>
      <div className="flex items-center">
        {kids.length ? (
          <CollapsibleTrigger aria-label={t('foldToggleSubs')} className="flex size-5 shrink-0 items-center justify-center rounded-sm text-sidebar-foreground/60 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground">
        ...
        <SidebarMenuButton className="min-w-0 flex-1" tooltip={f.name} onContextMenu={(e) => ctx.menu(e, f)} onClick={() => ctx.apply(f.id)}>
          <Folder />
