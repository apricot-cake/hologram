...
export function KindMenuHost() {
  ...
  useEffect(() => {
    if (!menu) return;
    const onDoc = (e) => { if (popRef.current && popRef.current.contains(e.target)) return; window.corpusKindMenu.close(); };
    const onKey = (e) => { if (e.key === 'Escape') window.corpusKindMenu.close(); };
  ...
  const pick = (row) => { window.corpusKindMenu.close(); menu.onPick(row.kind); };
  const rename = (e, kind) => { e.stopPropagation(); window.corpusKindMenu.close(); menu.onRename(kind); };

  return createPortal(
    <div className="fold-menu kind-menu show" ref={popRef}>
      <div className="fm-head">{menu.header}</div>
      {menu.rows.map((row, i) => row.sep ? (
        ...
        <div key={i} className="fm-row" onClick={() => pick(row)}>
          ...
          {row.checked && <span className="fm-check"><Check /></span>}
        </div>
      ))}
    </div>,
    document.body
