...
export function App({ store }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e) => { if (e.key === 'Escape') store.set(false); };
  ...
  const matchCount = searching ? matchIds.size : SECTIONS.length;
  const isHidden = (id) => (searching ? !matchIds.has(id) : id !== activeId);
  const tocHidden = (id) => (searching ? !matchIds.has(id) : false);
  const pickPage = (id) => { setActiveId(id); setQuery(''); };

  return (
    <SearchContext.Provider value={q}>
      <div className="settings-view" onClick={(e) => { if (e.target === e.currentTarget) store.set(false); }}>
        <div className="settings-view-inner">
          ...
          <div className="settings-empty" hidden={!(searching && matchCount === 0)}>{t('settingsNoMatch')}</div>
            ...
            <nav className="settings-toc">
              {SECTIONS.map((s) => (
                <button
                  key={s.id}
                  type="button"
                  className={'toc-item' + (!searching && s.id === activeId ? ' active' : '')}
                  hidden={tocHidden(s.id)}
                  onClick={() => pickPage(s.id)}
                >
                  {t(s.titleKey)}
                </button>
              ))}
